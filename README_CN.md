...
每天，MyxAI 在本地读取你的浏览记录和近期对话，
分析你真正关心什么，全网搜索最有价值的内容，
生成一份**只属于你的私享会**。

> 🔒 所有数据处理默认在本地进行，不上传至任何服务器。浏览记录仅在你的设备上读取和分析。
...
所有报告集中在一个页面 — 每日私享、邮件简报、自定义应用。按时间范围筛选、标记已读、在页面内直接浏览，不用来回切换。
...
把任何对话变成可重复执行的应用——专注于网络搜索和内容生成类任务。你的 AI，你的工作流。

### 🛡️ 安全架构

安全是 MyxAI 的第一设计原则，贯穿对话和应用的每一层。

- **绝对禁止删除** — 整个系统内无任何文件删除操作。所有"删除"均移至回收站（`~/.nanobot/trash/`），可随时恢复
- **本地操作需确认** — 对话中涉及文件系统的操作，AI 必须先展示执行计划，用户确认后才执行
- **高危操作拒绝** — 删除文件、格式化磁盘、清空回收站等破坏性操作，即使用户要求也会被拒绝
- **工具级拦截** — 危险命令在执行前被实时拦截，从源头阻断风险
- **自定义应用沙箱** — 自定义应用仅能使用网络搜索工具，无法执行任何本地操作
- **本地处理** — 所有数据处理默认在本地进行，不上传至任何服务器
...
```
myai/
├── app.py               # Flask 后端 + pywebview 窗口 + 安全拦截
├── requirements.txt
├── apps/
│   ├── daily_digest.py  # 每日私享 pipeline
│   ├── email_summary.py # 邮件简报（IMAP + AI）
│   ├── custom_app.py    # 自定义应用框架
│   ├── web_search.py    # 多引擎搜索 + 配额管理
│   ├── safe_fs.py       # 安全文件操作（零删除，移至回收站）
